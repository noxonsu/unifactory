...
const CustomListRow = memo(function CustomListRow({ list }: { list: TokenList; key: number }) {
      ...
      <Column style={{ flex: '1' }}>
        ...
        <RowFixed mt="4px">
          <StyledListUrlText active={false} mr="6px">
            {list.tokens.length} {list.tokens.length === 1 ? 'token' : 'tokens'}
          </StyledListUrlText>
        </RowFixed>
      </Column>
    </RowWrapper>
            ...
            <RowBetween>
              ...
              {isImported ? (
                <RowFixed>
                  ...
                  <TYPE.body color={theme.text2}>{t('loaded')}</TYPE.body>
                </RowFixed>
              ) : (
                <ButtonPrimary
                  style={{ fontSize: '14px' }}
